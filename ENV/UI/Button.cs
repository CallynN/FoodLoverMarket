    ...
    public partial class Button : Firefly.Box.UI.Button, ICanShowCustomHelp
    {
        ...
        class FiveStateButtonWithText : DisplayStrategy
        {
            ...
            public FiveStateButtonWithText(Button parent, bool doesntHaveText)
            {
                ...
                _ip = new ImageSeperator((ip) => _parent._loadImage(ip));
                _parent = parent;
        ...
        class IconAndText : DisplayStrategy
        {
            ...
            public Image GetImage(string imageLocation)
            {
                ...
                using (var g = Graphics.FromImage(image))
                {
                    ...
                    var iconSize = _iconAlignment == ContentAlignment.TopCenter && !string.IsNullOrEmpty(_iconLocation) ? 32 : 16;

                    var icon = ENV.Common.GetImage(path, new Size(iconSize, iconSize));

                    if (icon != null)
                    {
                        if (_iconAlignment != ContentAlignment.TopCenter && icon.Width > iconSize)
                        {
                            var im = new Bitmap(iconSize, iconSize);
                            using (var g1 = Graphics.FromImage(im))
                            {
                                g1.DrawImage(icon, 0, 0, iconSize, iconSize);
                            }
                            icon = im;
                        }
                        iconSize = icon.Width;
