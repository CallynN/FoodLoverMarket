...
namespace ENV
{
    public partial class UserMethods
    {
        ...
        {
            if (text == null || textToLookFor == null) return null;
        ...
        public Text StrBuild(Text format, params Text[] args)
        {
            ...
            return result.ToString() + value + suffix;
        }
                        ...
                        if (r.Read())
                        {
                            for (int i = 0; i < r.FieldCount; i++)
                            {
                                if (i < resultColumnsIndexes.Length)
                                    VarSet(resultColumnsIndexes[i], r[i]);
                            }
                        }
                        return true;
        ...
        class FilterLoader : IColumnSpecifier
        {
            ...
            public void DoOnColumn(TypedColumnBase<Date> column)
            {
                ...
                if (o != null)
                {
                    try
                    {
                        Result.Add(column.IsEqualTo(Types.Date.Parse((string)o, "YYYMMDD")));
                        return;
                    }
                    catch
                    {
        ...
        public Bool IsDefault(Number selecedColumnIndex)
        {
            ...
            ColumnBase column = GetColumnByIndex(selecedColumnIndex);
            if (column != null)
                return column.Value == null ? column.DefaultValue == null :
        ...
        public Number ControlSelectProgram(Number controlId)
        {
            ...
            if (x != null && x.ExpandClassType != null)
            {
                ControllerBase.SendInstanceBasedOnTaskAndCallStack(t, y =>
                {
                    ...
                    result = y._application.AllPrograms.IndexOf(x.ExpandClassType);
                });
        ...
        public Bool EOP(Number generation, Number ioIndex)
        {
            ...
            if (tpw != null)
                return tpw.HeightUntilEndOfPage < 0 || tpw.NewPageOnNextWrite || printerWriter.PageEnded;
            return false;
        }
        ...
        public Number WinBox(Number generation, Text dimention)
        {
            if (generation == null || dimention == null)
                return null;
            if (Common._suppressDialogForTesting && dimention.ToUpper().Trim() == "H")
                return 700;
            ITask task = GetTaskByGeneration(generation);
            if (task != null)
            {
                ...
                if (form != null)
                {
                    ...
                    switch (dimention.ToUpper(CultureInfo.InvariantCulture))
                    {
                        case "X":
                            return f.Left * form.HorizontalExpressionFactor / form.HorizontalScale;
                        case "Y":
                            return f.Top * form.VerticalExpressionFactor / form.VerticalScale;
                        case "W":
                            return f.ClientSize.Width * form.HorizontalExpressionFactor / form.HorizontalScale;
                        case "H":
                            return f.ClientSize.Height * form.VerticalExpressionFactor / form.VerticalScale;
                    }
                }
            }
            return Number.Zero;
        ...
        public Bool CtrlGoTo(Text controlName, Number gridRowNumber, Number generation)
        {
            ...
            if (control != null && (cb != null ? cb.Visible : control.Visible))
            {
                ...
                while (p != null && p != f)
                {
                    ...
                    if (gridRowNumber > 0)
                    {
                        if (gridRowNumber - 1 <= g.GetIndexOfLastRowOfData())
        ...
        object UDF(Text f, object[] args, System.Runtime.InteropServices.CallingConvention callingConvention)
        {
            ...
            if (_as400SpecialFunctions != null &&
                (f.ToString().ToUpperInvariant().StartsWith("@MGEAC32") || f.ToString().ToUpperInvariant().StartsWith("MGEAC32")))
            {
            ...
            try
            {
                ...
                else if (dllName == "MAGICTKN")
                {
                    ...
                    {
                        var seperator = args[3].ToString().Trim();
                        if (seperator == "")
                            seperator = ",";

                        return StrToken(args[1].ToString(), StrTokenIdx(args[0].ToString(), args[2].ToString(), seperator), seperator);
                    }
                }
            ...
            try
            {
                ...
                switch (m)
                {
                    ...
                    case "DZ_EZ32":
                        ...
                        break;
                    case "USER32":
                        if (GetFunctionNameFrom(moduleName).ToUpperInvariant() == "GETSYSTEMMETRICS")
                        {
                            argumentTypes = "44";
                            var x = args;
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (x.Length < s.Length)
                                            ca[x.Length] = '\0';
                                        argumentsForDll.Add(new System.Text.StringBuilder(new string(System.Text.Encoding.Default.GetChars(LocalizationInfo.Current.InnerEncoding.GetBytes(ca))), 32000));
                                    }
                                }
                                break;
                            case 'L':
            ...
            : this(bp._businessProcess)
        {
            ...
            e.AddObjectForFunctions((add) => add(bp));
            ...
            : this(uic._uiController)
        {
            ...
            e.AddObjectForFunctions((add) => add(uic));
        ...
        const string CONTEXT_NAME_KEY = "CONTEXT_NAME";
        ...
        public Bool BufSetBlob(Number byteArrayColumnIndex, int startIndex, byte[] value, int storageType, bool storeAsPointer)
        {
            ...
            if (bac == null)
                return false;
            if (storageType == 1)
                throw new ArgumentException("Parameter value not supported", "storageType");
            if (storeAsPointer)
        ...
        public Bool DragSetData(Text data, Number format, Text userDefinedFormat)
        {
            try
            {
                ...
                ControlBase.DragDropData.SetData(dataFormat, dataFormat == DataFormats.FileDrop ? (object)new string[] { data } : data.ToString());
                return true;
            }
            catch (Exception ex)
            {
        ...
        public Text DropGetData(Number format, Text userDefinedFormat)
        {
            try
            {
                ...
                if (x is string[]) return string.Join(FilesSeperator, (string[])x);
        ...
        public Number RqReqLst(Text serviceName, Text requestIdMin, Text requestIdMax, Text password)
        {
            ...
            var result = new Remoting.HttpApplication(UserSettings.GetApplicationServerUrl(serviceName)).GetRequests(Number.Parse(requestIdMin), Number.Parse(requestIdMax), false);
        ...
        public static string FilesSeperator
        {
            get
            {
                if (UserSettings.Version10Compatible)
        ...
        public object ToObject(ArrayColumn<Text[]> value)
        {
            if (value == null)
                return System.Reflection.Missing.Value;
            var firstDimentionLength = value.Length;
            var secondDimentionLength = 0;
            foreach (var arr in value.Value)
            {
                if (arr.Length > secondDimentionLength)
                    secondDimentionLength = arr.Length;
            }
            var result = new object[firstDimentionLength, secondDimentionLength];
            for (var i = 0; i < firstDimentionLength; i++)
