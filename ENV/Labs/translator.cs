    ...
    public class Translator
    {
        ...
        static string _key
        {
            get
            {
                if (__key == null)
                {
                    if (System.IO.File.Exists(@"c:\firefly\googleApi.Key"))
                    {
                        _translationFile = @"c:\firefly\translateCache";
                        __key = System.IO.File.ReadAllText(@"c:\firefly\googleApi.Key");
                    }
                    else if (!_failed && System.IO.File.Exists(@"\\10.100.51.63\c\firefly\googleapi.key"))
                    {
                        _translationFile = @"\\10.100.51.63\c\firefly\translateCache";
                        __key = System.IO.File.ReadAllText(@"\\10.100.51.63\c\firefly\googleapi.key");
                    }
                    _failed = true;
                }
                return __key;
