...
namespace ENV
{
    public class MenuManager
    {
        ...
        Dictionary<int, ToolStrip[]> _menues = new Dictionary<int, ToolStrip[]>();
        ...
        public void MapMainMenu(int menuKey, params ToolStrip[] menuControls)
        {
            ...
            _menues.Add(menuKey, menuControls);
            if (_menuForm != null)
            {
        ...
        bool IsMenuActive(ToolStrip owner)
        {
            if (owner == null)
                return false;
            if (!_menues.ContainsKey(_activeMenu))
                return false;
            foreach (var ts in _menues[_activeMenu])
            {
                if (owner == ts)
                    return true;
            }
        ...
        {
            _showMenu = showMenu;
            if (!_menues.ContainsKey(menuKey))
            {
                ...
                if (x != null)
                {
                    ...
                    ms.ResumeLayout();
                    ms.PerformLayout();
                    _menues.Add(menuKey, arr.ToArray());
                }
        ...
        public Bool Deactivate(int menuKey)
        {
            ...
            Firefly.Box.Context.Current.InvokeUICommand(
                () =>
                {
                    foreach (var toolStripse in _menues[menuKey])
                    {
                        if (_menuForm != null)
        ...
        {
            foreach (var toolStripse in _menues)
            {
                        ...
                        if (b != null)
                        {
                            ...
                            if (menuActive)
                            {
                                if (v != _item.Available)
                                    fixSeperators = () => FixSeperators(_item.Owner);
                            }
                        }
        ...
        static void FixSeperators(ToolStrip toolStrip)
        {
            var x = 0;
            ...
            while (x < toolStrip.Items.Count && toolStrip.Items[x] is ToolStripSeparator)
            {
                var seperator = toolStrip.Items[x];
                var seperatorShouldBeVisible = false;
                x++;
                while (x < toolStrip.Items.Count && !(toolStrip.Items[x] is ToolStripSeparator))
                    if (toolStrip.Items[x++].Available) seperatorShouldBeVisible = true;
                seperator.Available = seperatorShouldBeVisible;
            }
        }
    ...
    namespace UI.Menus
    {
        ...
        public class Seperator : IMenuEntry
        {
        ...
        public class ManagedCommand : MenuEntry
        {
            ...
            protected override void DecorateMenuItem(ToolStripItem item, MenuManager mm)
            {
                if (_commandsAvailableInFilterMode.Contains(_command))
                {
                    ...
                    TemplateModeFilter.SetTemplateModeMenues(item);
                }
            }
        ...
        public class MenuEntry : IMenuEntry, IEnumerable<IMenuEntry>
        {
            ...
            public void AddToToolbar(MenuManager mm, Action<ToolStripItem> add)
            {
                ...
                item.ToolTipText = _caption.Replace("&", "");
