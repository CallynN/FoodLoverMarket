...
namespace ENV
{
    public class PathDecoder
    {
        ...
        internal string ReplaceDollarAndDecode(string source)
        {
            Action<Dictionary<string, string>> doIT = (dict) =>
            {
                foreach (var item in dict)
                {
                    var i = source.ToUpper().IndexOf("$" + item.Key);
                    if (i >= 0)
                    {
                        var j = i + item.Key.Length + 1;
                        if (j == source.Length || (!char.IsLetterOrDigit(source[j])))
                        {
                            source = source.Remove(i, item.Key.Length + 1).Insert(i, item.Value);
                        }
                    }
                }
            ...
            return InternalDecodePath(source, false);
        }
