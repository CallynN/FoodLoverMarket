...
namespace ENV.Data.DataProvider
{
    ...
    public class DynamicSQLSupportingDataProvider : ISQLEntityDataProvider, ISupportsGetDefinition, ICanGetDecoratedISSQLEntityDataProvider
    {
        ...
        class mySqlDataProviderForSqlEntity : IEntityDataProvider
        {
            ...
            class NormalColumnInQuery : ColumnInQuery
            {
                ...
                {
                    rowStorage.SetValue(_column, new DataReaderValueLoader(dataReader, _index, DummyDateTimeCollector.Instance));
                }
            }
    ...
    class BridgeToISQLEntityDataProvider : ISQLEntityDataProvider
    {
        ...
        public BridgeToISQLEntityDataProvider(SQLClientEntityDataProvider p) :
            this(p, p.CreateCommand, s => new MssqlTable(p.GetEntityName(s), s), false, p.ProcessException, new UserDbMethods.Default(), ((SQLDataProviderHelperClient)p).GetDataReaderValueLoader, p.GetEntityName, p.IsClosed)
        {
        }


        public BridgeToISQLEntityDataProvider(OracleClientEntityDataProvider p) :
            this(p, p.CreateCommand, s => new OracleTable(p.GetEntityName(s)), true, p.ProcessException, new UserDbMethods.OracleDbMethods(), ((SQLDataProviderHelperClient)p).GetDataReaderValueLoader, p.GetEntityName, p.IsClosed)
        {
            _userMethods = new UserDbMethods.OracleDbMethods();
        }
        public BridgeToISQLEntityDataProvider(OdbcEntityDataProvider p) :
            this(p, p.CreateCommand, s => new OdbcTable(p.GetEntityName(s), p.ColumnNameWrapper), false, p.ProcessException, new UserDbMethods.Default(), ((SQLDataProviderHelperClient)p).GetDataReaderValueLoader, p.GetEntityName, p.IsClosed)
        {
        }
