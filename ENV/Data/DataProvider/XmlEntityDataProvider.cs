...
namespace ENV.Data.DataProvider
{
        ...
        class XmlColumnProviderHelper : XmlDataProviderHelper
        {
            ...
            System.Text.Encoding GetEncoding()
            {
                ...
                if (c != null)
                    ...
                    else if (c.ContentType == ByteArrayColumnContentType.Ansi && !SaveAnsiAsUTF8)
                        return LocalizationInfo.Current.OuterEncoding;
                ...
                if (si != null)
                {
                    ...
                    }
                    {
                        for (int i = parent.ChildNodes.Count - 1; i >= 0; i--)
                        {
                            ...
                            if (GetSchemaItemFor(item, e)._order <= cOrder)
                            {
                ...
                public IRow Insert(IEnumerable<ColumnBase> columnsOriginal, IEnumerable<IValue> valuesOriginal, IRowStorage storage, IEnumerable<ColumnBase> selectedColumns)
                {
                    ...
                    for (int i = columns.Count - 1; i >= 0; i--)
                    {
                        ...
                        else if (!columns[i].WasSet)
                        {
                            ...
                            if (_entity.AttributeColumns.Contains(columns[i]) && tc != null && !Text.IsNullOrEmpty(tc.DefaultValue))
                            {
                            }
                            else
                            {
            ...
            void DoOnChildElement(XmlEntity e, XmlNode el, string columnName, bool isAttrubite, Action<XmlNode> whatToDo, bool shouldCreate, bool shouldCreatePathParents)
            {
                ...
                else
                {
                    ...
                    if (isAttrubite)
                    {
                        ...
                        {
                            var y = _doc.CreateAttribute(n);
                            whatToDo(y);
                            iterate.Attributes.Append(y);
    ...
    public class XmlEntity : Entity
    {
        ...
        {
            if (ReferenceEquals(entity._sourceColumn, null) || ReferenceEquals(entity._sourceColumn.Value, null))
