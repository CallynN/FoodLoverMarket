...
namespace ENV.Data.DataProvider
{
    public class BtrieveDataProvider : IEntityDataProvider
    {
        ...
        class OpenedBtrieveFile
        {
            ...
            public void SendColumnDefinition(ColumnBase column, Action<int, int, int> toMe)
            {
                var cd = _columnDefsByName[column.Name];
                toMe(cd.DataTypeCode, cd.Offset, cd.Size);
            }
            ...
            class myRowsReader : IRowsReader
            {
                ...
                bool ReadNext(bool startWithCurrent, bool reacquirePosition)
                {
                    if (_getNextExtendedData == null)
                    {
                        ...
                        w.AddEmpty(2); // Maximum Reject Count
                        ...
                        cacheSize = _lockAllRows ? 1 : _file.GetMaxRowLength() > 0 ? Math.Min((ushort.MaxValue - 2) / (_file.GetMaxRowLength() + 6), Math.Max(1200 / _file.GetMaxRowLength(), cacheSize)) : cacheSize;
                        ...
                        if (_usingKey)
                        {
                            ...
                            foreach (var s in _sort.Segments)
                            {
                                ...
                                _file.SendColumnDefinition(s.Column,
                                    (dataTypeCode, ofst, columnSize) =>
                                    {
                                        size = columnSize;
                                        _file.GetColumnSize(s.Column);
                                        if (bSort != null)
