    ...
    public abstract class AbstractUIController : ControllerBase, IHaveUserMethods
    {
        ...
        protected internal void BindOrderBy(Func<Sort> orderBy)
        {
            ...
            x.Invokes +=
                y =>
                {
                    if (keepModifiedSort.Value && _lastSetOrderBy != OrderBy) return;
                    _setOrderBy();
                };
        }
        ...
        {
            if (From is ENV.Data.DynamicSQLEntity)
                _uiController.ReloadDataOnReEntry = true;
        }
                ...
                else
                {
                    if (sortForm == null)
                    {
                        if (!e.Done && sortStart.ElapsedMilliseconds > _interval)
                        {
                            ...
                            {
                                sortForm = new UI.Form { Text = _title };
                                ...
                                Common.RunOnContextTopMostForm(topMost =>
                                {
                                    if (topMost.IsMdiContainer)
                                        sortForm.MdiParent = topMost;
                                    else
                                        sortForm.Owner = topMost;
                                });
                                var parent = _view();
                                if (parent == null)
                                ...
                                else
                                {
                                    ...
                                    sortForm.Location = new System.Drawing.Point(parent.Left + ((parent.Width - sortForm.Width) / 2), parent.Top + ((parent.Height - sortForm.Height) / 2));
                                    if (sortForm.MdiParent != null)
                                    {
                                        if (sortForm.Right > sortForm.MdiParent.ClientSize.Width)
                                        {
                                            sortForm.Left = (sortForm.MdiParent.ClientSize.Width - sortForm.Width) / 2;
                                        }
                                    }
                                }
                                sortForm.Show();
