...
namespace ENV.Utilities
{
    class EvaluateExpressions
    {
        ...
        class MyMethod
        {
            MethodInfo _method;
            object _instance;
            public MyMethod(MethodInfo m, object instance)
            {
                _method = m;
                _instance = instance;
            }
                ...
                foreach (var f in _objects)
                {
                    f(item =>
                    {
                        ...
                        foreach (var method in item.GetType().GetMethods())
                        {
                            ...
                            if (!result.TryGetValue(method.Name.ToUpper(CultureInfo.InvariantCulture), out methodsByname))
                            {
                                result.Add(method.Name.ToUpper(CultureInfo.InvariantCulture), methodsByname = new List<MyMethod>());
                            }
                            methodsByname.Add(new MyMethod(method, item));
                        }
                    });
        ...
        class ExpressionStringVisitor : NodeVisitor
        {
            ...
            public void Function(string name, ExpressionNode[] parameters)
            {
                ...
                {
                    var v = new ExpressionStringVisitor(_parent);
                    p.Visit(v);
                    if (Result.Length > 2)
                        Result += ", ";
                    Result += v.Result;
        ...
        class TreeVisitor : NodeVisitor
        {
            ...
            public void Operator(string name, ExpressionNode leftExpression, ExpressionNode rightExpression)
            {
                ...
                switch (name.ToUpper(CultureInfo.InvariantCulture))
                {
                    ...
                    default:
                        throw new Exception("Unknown uperator " + name);
                }
            }
            ...
            {
                return new Exception(string.Format("Cant convert {0} of type {1} to {2}", _result, expectedType.Name,
                                                   typeof(Number).Name));
            }
        ...
        object EvaluateFunction(string name, List<TreeVisitor> args)
        {
            ...
            throw new Exception("No overload for " + name + " was found that matches the parameters received");
        }
