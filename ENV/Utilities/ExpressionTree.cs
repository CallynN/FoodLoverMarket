...
namespace ENV.Utilities
{
    ...
    class ExpressionTree : TokensConsumer
    {
        ...
        {
            DecimalNode dn = null;
            var c = Number.DecimalSeperator.Value;
            if (c != '.')
            {
                Number.DecimalSeperator.SetContextValue('.');
            }
            try
            {
            ...
            {
                if (c != '.')
                    Number.DecimalSeperator.SetContextValue(c);
            }
            _state.Value(dn);
        }
        ...
        class ConsumerState
        {
            ...
            public virtual void Seperator(char c)
            {
                throw new ExpressionParseException("Didn't anticipate seperator " + c);
            }
        ...
        class StartOfExpression : ConsumerState
        {
            ...
            public override void Seperator(char c)
            {
                if (c == '(')
        ...
        class NotState : ConsumerState
        {
            ...
            public override void Seperator(char c)
            {
                if (c == '(')
                {
                    (_parent._state = new FunctionState(_parent, _name, _parentConsumer)).Seperator(c);
                }
            }
            public override void Value(ExpressionNode value)
            {
                    ...
                    public override void Seperator(char c)
                    {
                        switch (c)
                        {
                            ...
                            case ']':
                                ...
                                _parentConsumerState.Seperator(c);
                                break;
                            case ',':
                                _parentConsumerState.AddValue(_value);
        ...
        public void Seperator(char c)
        {
            StringParser.Debug(c, "Seperator-");
            _state.Seperator(c);
        }
