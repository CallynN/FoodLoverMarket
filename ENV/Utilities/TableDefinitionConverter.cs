...
namespace ENV.Utilities
{
    public class TableDefinitionConverter
    {
        ...
        {
            return new IndexDefinition { Unique = idx.Unique, Name = idx.Name, Segments = idx.Segments.Select(s => DefOf(s)).ToArray() };
        }

        static SegmentDefinition DefOf(SortSegment seg)
        {
        ...
        {
            var idx = new Sort { Unique = idxDef.Unique, Name = idxDef.Name };
            Array.ForEach(idxDef.Segments, s => idx.Segments.Add(new SortSegment(e.Columns[s.ColumnName],
