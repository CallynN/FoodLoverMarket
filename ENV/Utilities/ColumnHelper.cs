...
namespace ENV.Utilities
{
    ...
    class ControlHelper
    {
        ...
        public ControlHelper(InputControlBase control)
        {
            _control = control;

            _control.Load += () =>
            {
                var col = Common.GetColumn(_control);
                if (col != null)
        ...
        void MyExpand()
        {
            ColumnHelper.DoExpand(_expand, Common.GetColumn(_control));
        }
        ...
        public void ClearExpandEvent()
        {
            ...
            _expand = null;
            _control.Expand -= MyExpand;
            var x = Common.GetColumn(_control) as IENVColumn;
            if (x != null)
                x.ClearExpandEvent();
        }
        ...
        {
            SetFocusedControlOfParentForm(_control);
            ...
            ProvideStatusTip(x =>
            {
                ...
                if (!_registeredFocusEvents && _control.Controls.Count > 0)
                {
                    _control.Controls[0].GotFocus += ControlHelper_GotFocus;
                    _control.Controls[0].LostFocus += ControlHelper_LostFocus;
                    _registeredFocusEvents = true;
                }
            });


            ENV.Advanced.LevelProvider.StartEnterControlContext(controlEvent, _control);
            if (_readonlyFoeExistingRows && !_control.ReadOnly && !ENV.UserMethods.Instance.Stat(0, "C"))
            {
                var cb = _control as ENV.UI.ComboBox;
        ...
        public void ControlLeave(Action controlEvent)
        {
            ...
            ENV.Advanced.LevelProvider.StartLeaveControlContext(controlEvent, _control);
        ...
        {
            ENV.Advanced.LevelProvider.StartChangeControlContext(controlEvent, _control);
        }
        public void ControlInputValidation(Action controlEvent)
        {
            ENV.Advanced.LevelProvider.StartInputValidationControlContext(controlEvent, _control);
        }
