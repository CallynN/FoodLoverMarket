...
namespace ProfilerUI
{
    interface TreeItem
    {
        void Set(string name, string shortName, long calls, long duration, bool isDb, long start, long end);
    ...
    public class ProfilerTree
    {
        ...
        public ProfilerTree(string profilerTypeName, Func<bool> save, Func<bool> saved, string name = null)
        {
            ...
            sortContextItem.Click += delegate
            {
                ...
                sortContextItem.Text = "Sort by " + (sortByTime ? "Duration" : "Chronological order");
                var selectedNode = _tv.SelectedNode;
                _tv.TreeViewNodeSorter = new Sorter(sortByTime);
                _tv.Sort();
        ...
        class TreeNodeBridgeToTreeItem : System.Windows.Forms.TreeNode, TreeItem
        {
            ...
            public long _childCalls, _childDuration, _dbDuration, _calculatedCount;
            ...
            public void CompleteCalc()
            {
                ...
                string statistis = "";
                if (_duration > 0)
                    statistis = DurationToString(_duration);
                if (!_trace)
                {
                ...
                string text = "";
                if (_duration != _totalDuration && _duration > 0 && !_trace)
                    text = PercentToString(_duration, _totalDuration) + " • ";
            ...
            public void Set(string name, string shortName, long calls, long duration, bool isDb, long start, long end)
            {
                _isDb = isDb;
                _name = name;
                _count = calls;
            ...
            internal int compareTo(object z, bool byTime)
            {
    ...
    class MagicEntry
    {
        ...
        {
            x.Set(Name, Name, Count, Duration, Db, 0, 0);
