...
namespace ENV.IO.Advanced
{
    public class Reader : ReaderInterface, IDisposable, IStringToByteArrayConverter
    {
        ...
        void ReaderInterface.ReadSection(int width, char seperator, Action<ValueProviderDelegate> setTextValue)
        {
            new mySeperatedReader(ReadLine(width), seperator, this).SendValuesTo(setTextValue);
        }

        void ReaderInterface.ReadSectionDoubleSeperator(int width, char seperator, Action<ValueProviderDelegate> setTextValue)
        {
            var x = ReadLine(width);
            var s = x.Split(seperator);
            if (s.Length > 1)
        ...
        class mySeperatedReader
        {
            ...
            public mySeperatedReader(string line, char seperator, Reader parent)
            {
