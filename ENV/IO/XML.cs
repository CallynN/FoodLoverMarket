    ...
    public class XmlHelper
    {
        ...
        System.Xml.XmlElement FindElement(string path)
        {
            ...
            if (result == null)
                return null;
            bool first = true;
            char seperator = DetermineSeperator(path);

            if (path.Length == 0)
                return null;
            if (path[0] == seperator)
                path = path.Substring(1);
            string[] splittedPath = path.Split(seperator);
        ...
        internal void WriteTo(ENV.Data.ByteArrayColumn column)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                using (var s = new StreamWriter(ms, column.ContentType == ByteArrayColumnContentType.Ansi ? LocalizationInfo.Current.OuterEncoding : e))
                {
                    DoSave(s, e);
        ...
        internal static char DetermineSeperator(Text path)
        {
        ...
        public Number Count(Text path)
        {
            if (path == null)
                return null;
            var seperator = DetermineSeperator(path);
            var parent = _document.ChildNodes;
            var nameOfCountedNode = path;
            if (path.IndexOf(seperator) > -1)
            {
                string pathOfParent = SubstringUntillLastApearanceOf(path, seperator);
                nameOfCountedNode = SubstringAfterLastApearanceOf(path, seperator);
        ...
        public Number Add(Text path, Text attributeName, Text value, Text beforeOrAfter, Text referencedElementName, Bool convertToXml)
        {
            ...
            try
            {
                ...
                if (path.StartsWith(seperator.ToString()))
                    path = path.Substring(1);
                if (string.IsNullOrEmpty(attributeName))
                {
                    if (convertToXml)
                        value = ENV.UserMethods.Instance.InternalXMLVal(value, true);
                    string nameOfItem = SubstringAfterLastApearanceOf(path, seperator);
                    var de = _document.DocumentElement;
                    var e = _document.CreateElement(nameOfItem, de != null ? de.NamespaceURI : "");
