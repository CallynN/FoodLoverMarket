...
namespace ENV.IO
{
    public class TextSection : ENV.Printing.ReportSection
    {
        ...
        /// </summary>
        /// <remarks>The string is separated by the <paramref name="seperator"/>.
        /// Used mainly for comma separated reading.</remarks>
        /// <param name="reader">The <paramType/> from which to read</param>
        /// <param name="seperator">The separator to use</param>
        public void ReadSeparated(Reader reader, char seperator)
        {
            ApplyBinding();
            ...
            if (BeforeRead != null)
                BeforeRead();
            ((ReaderInterface)reader).ReadSection(WidthInChars, seperator,
        ...
        public void ReadDoubleSeparated(Reader reader, char seperator)
        {
            ApplyBinding();
            if (_sortedControls == null)
            {
            ...
            if (BeforeRead != null)
                BeforeRead();
            ((ReaderInterface)reader).ReadSectionDoubleSeperator(WidthInChars, seperator,
                delegate (ValueProviderDelegate obj)
                {
                    if (i >= _sortedControls.Count)
                        return;
        ...
        public void ReadDoubleSeperated(Reader reader, char seperator)
        {
            this.ReadDoubleSeparated(reader, seperator);
        }
