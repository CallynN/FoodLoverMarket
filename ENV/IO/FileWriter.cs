...
namespace ENV.IO
{
    public class FileWriter : Writer, IOByName, ITemplateEnabled
    {
            ...
            : this()
        {
            ...
            // method name: FileWriter (to cater for users running multiple reports on multiple sessions)
            // created: 05 June 2020
            // Author: SundariAngappan
            // Revisions: 

            if (!fileName.Contains("."))
            {
               // fileName = fileName + "(" + ProcessIdentifier.sessionId + ")";
                _createTextWriter = _fileWriterFactory(fileName, encoding, this);

            }
            else if (fileName.Contains("."))
            {
                fileName = fileName.Insert(fileName.LastIndexOf("."), "(" + ProcessIdentifier.sessionId + ")");
                _createTextWriter = _fileWriterFactory(fileName, encoding, this);

            }
            else if (fileName.Contains("liv"))
            {
                _createTextWriter = _fileWriterFactory(fileName, encoding, this);
            }
          //  _createTextWriter = _fileWriterFactory(fileName, encoding, this);
